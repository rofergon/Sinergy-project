...
async function fetchBinanceKlines(symbol, startTime, endTime) {
  ...
  while (retries > 0) {
    try {
      ...
      if (!res.ok) throw new Error(`Binance error: ${res.statusText} - ${await res.text()}`);
...
async function main() {
  for (const [asset, binanceSymbol] of Object.entries(symbolsToFix)) {
    const rows = db.prepare(`
      ...
      SELECT prev_ts as start, ts as end, gap_seconds FROM Gaps WHERE gap_seconds > 60 ORDER BY start ASC
    `).all(asset);
    console.log(`Found ${rows.length} gaps for ${asset}`);
    for (const gap of rows) {
      ...
      while (currentStart < gapEnd) {
        ...
        db.exec("COMMIT");
        console.log(`  Inserted ${count} for ${asset} [${new Date(currentStart).toISOString()} to ${new Date(currentEnd).toISOString()}]`);
